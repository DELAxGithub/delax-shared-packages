...
# =============================================================================
# iOS Xcode Auto Build & Fix Script
# DELAX技術遺産 - iOS自動ビルド&修正システム
# =============================================================================

# --- Configuration ---
PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
XCODE_PROJECT="${XCODE_PROJECT:-Myprojects/Myprojects.xcodeproj}"
SCHEME="${SCHEME:-Myprojects}"
...
CONFIG_FILE="${CONFIG_FILE:-config/auto-fix-config.yml}"
MAX_ATTEMPTS=5
CURRENT_ATTEMPT=0
...
extract_errors() {
    ...
    "${PROJECT_DIR}/scripts/extract-xcode-errors.sh" "$BUILD_LOG" > "$ERROR_FILE"
...
generate_fix() {
    ...
    "${PROJECT_DIR}/scripts/claude-patch-generator.sh" "$ERROR_FILE" > "$PATCH_FILE"
...
apply_fix() {
    ...
    "${PROJECT_DIR}/scripts/safe-patch-apply.sh" "$PATCH_FILE"
    return $?
}
