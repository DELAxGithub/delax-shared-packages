...
# DELAX技術遺産 - ビルドエラーからClaude Code CLIを使ってパッチを生成
# =============================================================================
...
generate_project_context() {
    cat << EOF
# iOS App - Claude 4 Sonnet Context
...
gather_error_context() {
    ...
    for file in "${unique_files[@]}"; do
        # Try common iOS project paths
        for path in "$PROJECT_DIR/$file" "$PROJECT_DIR/src/$file" "$PROJECT_DIR/Sources/$file"; do
            if [[ -f "$path" ]]; then
                echo "" >&2
